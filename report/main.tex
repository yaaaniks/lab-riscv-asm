...
В соответствии с \textit{соглашением о вызовах функций RISC-V} первые четыре аргумента функции передаются через регистры \texttt{a0-a3}, а результат возвращается в регистре \texttt{a0}.
...
\hline
\texttt{s0} & x8 & Базовый адрес матрицы \\
\hline
...
\hline
\texttt{t1} & x6 & Вычисление смещения matrix[i][i] \\
\hline
\texttt{t2} & x7 & Текущий элемент matrix[i][i] \\
\hline
\texttt{a0} & x10 & Первый аргумент функции / возвращаемое значение \\
\hline
\texttt{a1} & x11 & Второй аргумент функции / аргумент для \texttt{ecall} \\
\hline
\texttt{a7} & x17 & Код системного вызова (\texttt{ecall}) \\
\hline
\end{tabularx}
\end{table}
...
Прежде чем описывать функциональную часть программы разместим входные и неизменяемые данные в сегменте \texttt{.data}.
...
Все инструкции программы разместим в сегменте \texttt{.text}; для глобальной видимости функций используется метка \texttt{.globl}.
В функции \texttt{findMinElement} вначале создадим стековый фрейм для сохранения регистров, которые будут изменяться. 
...
\begin{listing}[H] 
    ...
    loop:
        li t0, 6
        bge s2, t0, endloop       # if i >= 6 → exit

        # matrix[i][i] offset = (i*6 + i)*4 = i*7*4
        li t1, 7
        mul t1, t1, s2            # t1 = i * 7
        slli t1, t1, 2            # t1 *= 4
        ...
        blt t2, s1, updateMin    # if matrix[i][i] < min
        j next_i
...
Ниже приведён код функции \texttt{main}, которая вызывает функцию поиска минимального элемента и выводит результат на экран с использованием системных вызовов.
...
\begin{listing}[H] 
    ...
    main:
        ...
        la a0, matrix             # argument: pointer to matrix
        call findMinElement       # a0 <- result
...
Ниже в листинге \ref{lst:ccode} приведён код программы на языке C, решающей ту же задачу.
\begin{listing}[H]
...
Ниже приведены команды, использованные для сборки и запуска программы:
...
Далее код был переписан на языке ассемблере \textit{RISC-V} и проверен в симуляторе \textit{Venus}.


\section{Выводы}

В ходе выполнения лабораторной работы изучены базовые инструкции архитектуры RISC-V. 
Реализована программа на языке C и её эквивалент на языке ассемблера. 
Ознакомились с процессом сборки и запуска программ для архитектуры RISC-V. 
